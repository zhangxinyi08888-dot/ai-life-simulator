...
const RESTRUCTURE_COMPLETED_PATTERN = /重组协议.{0,10}(?:签署|生效)|还款方式(?:已经|已)?改为|期限(?:已经|已)?延长|银行(?:已经|已)?(?:批准|同意).{0,16}(?:展期|重组|先息后本|宽限期)/u;
const DEBT_COMPLETED_PATTERN = /(?:还清|结清)(?:了|全部|所有)?(?:房贷|贷款|债务|欠款)?|(?:房贷|贷款|债务|欠款)(?:已经|已)?清零|清偿完毕/u;
...
const EXACT_TOTAL_DEBT_PATTERN = /(?:个人(?:总)?|累计|当前|总计|整体|这笔)?(?:总)?(?:债务|负债)(?:余额)?(?:约|为|达到|升至|增加到|还有)?\s*(\d+(?:\.\d+)?)\s*万(?:元)?/gu;
