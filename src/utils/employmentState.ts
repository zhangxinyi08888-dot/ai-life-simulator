...
export function hasCompletedEmployerStartEvidence(value: string): boolean {
  ...
  const completedStart = /(?:正式|已经|已(?:经)?|开始|随后)[^。；]{0,24}(?:入职|到岗|上班|任职|担任)|(?:入职|到岗|上班)(?:[^。；]{0,16}(?:创业公司|公司|企业|机构|团队))?后(?:[，,。；]|(?:你|我|本人|主角|自己|便|就|开始|发现|负责|进入)|(?:的)?(?:前|头|最初|第)?[一二三四五六七八九十0-9]+(?:个)?(?:月|周|天|年))|(?:入职|到岗|上班)(?:的)?(?:第一|首)(?:天|周|个月)[，,]?(?:你|我|本人|主角|自己)?[^。；]{0,16}(?:发现|开始|负责|完成|参加|处理|接手|进入|坐在|见到|拿到)|(?:你|我|本人|主角|自己)[^。；]{0,12}(?:入职|到岗|上班|任职|担任)后|(?:正式)?加入[^。；]{0,24}(?:公司|企业|机构|团队)[^。；]{0,28}(?:担任|任职|负责|工作|职位|岗位)/u;
