...
assert.equal(generatedParagraphs.length <= 4, true);
...
assert.equal(rebalancedParagraphs.length, 4);
assert.equal(rebalancedParagraphs.join(""), oversizedExplicitParagraphs.replaceAll("\n\n", ""));
assert.equal(rebalancedParagraphs.every((paragraph) => paragraph.length < 200), true);
