...
const commitmentReviewEvent = LIFE_EVENTS_DATABASE.find((candidate) => candidate.id === "relationship_material_commitment_test");
assert.ok(commitmentReviewEvent);
const commitmentReviewPrompt = buildEventIntentPrompt(commitmentReviewEvent, storyContext);
assert.match(commitmentReviewPrompt, /不表示必须结婚/);
assert.match(commitmentReviewPrompt, /不得自动推进为同居、已婚或已有子女/);
