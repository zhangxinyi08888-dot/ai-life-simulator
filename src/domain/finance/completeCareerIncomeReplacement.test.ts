...
test("a salary migration supersedes a stale end for the same prior source", () => {
  const ledger = initializeFinancialLedger({ id: "career_migration", asOfAgeInMonths: 360, openingPosition: {
    incomeSources: ["salary", "legacy_salary"].map((id) => ({
      id, type: "salary" as const, displayName: id, monthlyNetAmountWan: 2, accrualPolicy: "monthly" as const,
      activeFromAgeInMonths: 300, status: "active" as const, linkedCareerStateId: "career_old",
      factStatus: "estimated" as const, evidence: []
  ...
  const proposals = completeCareerIncomeReplacementProposals({
    proposals: [
      {
        id: "migrate_salary", kind: "income_source_adjusted", effectiveAtAgeInMonths: 361,
        payload: {
        ...
        sourceOutcomeId: "choice", evidence: "你正式入职新公司，税后月薪2.2万元。", confidence: 1
      },
      {
        id: "stale_end_salary", kind: "income_source_ended", effectiveAtAgeInMonths: 362,
        payload: { incomeSourceId: "salary" }, sourceOutcomeId: "choice", evidence: "你正式入职新公司。", confidence: 1
      }
    ],
    currentLedger: ledger, currentCareerStateId: "career_old", transition, acceptedOutcomeId: "choice"
  });
  assert.equal(proposals.some((proposal) => proposal.id === "stale_end_salary"), false);
  assert.equal(proposals.some((proposal) => proposal.id === "migrate_salary"), true);
  assert.deepEqual(
