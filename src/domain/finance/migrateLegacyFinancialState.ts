...
export function migrateLegacyFinancialState(input: {
  ...
  const assetAccounts = [
    ...
    ...((input.openingFacts?.propertyMarketValueWan !== undefined
      ? input.openingFacts.propertyMarketValueWan
      : input.openingFacts
      ? 0
      ...
      type: "property" as const,
      displayName: "旧版房产聚合",
      marketValueWan: roundWan(input.openingFacts?.propertyMarketValueWan ?? state.propertyMarketValueWan),
      liquidity: "illiquid" as const,
      status: "active" as const,
