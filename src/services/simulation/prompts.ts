...
- description：150-250 字、具体写实的剧情正文；必须分成 2-4 个自然段，并在 JSON 字符串中用 \\n\\n 分隔；
...
startNode 要求：
- description：150-250 字，具体、干练、写实，包含现实事务和社会压力；分成 2-4 个自然段，并在 JSON 字符串中用 \\n\\n 分隔。
...
- 如果不是结局，请写 150-250 字现实冲突，避免金手指和无理倒霉；正文分成 2-4 个自然段，并在 JSON 字符串中用 \\n\\n 分隔。
${FINANCIAL_NARRATIVE_RULE}
...
要求：
- 写 150-250 字自然收束，结合最近选择、关系、事业、健康和长期方向；正文分成 2-4 个自然段，并在 JSON 字符串中用 \\n\\n 分隔。
...
请在此岁数开启完全不同的命运平行宇宙：
- description：150-250 字，突出新方向面临的现实磨练、物质局限和世俗博弈；分成 2-4 个自然段，并在 JSON 字符串中用 \\n\\n 分隔。
${FINANCIAL_NARRATIVE_RULE}
