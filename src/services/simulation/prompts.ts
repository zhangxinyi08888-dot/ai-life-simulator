...
- 期权授予：business_option_granted.payload.optionHolding 必须使用 instrumentType="stock_option"，包含 optionTerms.grantedUnits/vestedUnits/exercisedUnits/strikePriceWanPerUnit；授予时 personalCarryingValueWan 必须为 0。随后实际归属用 business_option_vested；有可靠公允单价后用 business_option_revalued，newCarryingValueWan 只能等于“剩余已归属数量 × max(公允单价-行权价,0) × (1-流动性折扣) × (1-实现风险折扣)”。
