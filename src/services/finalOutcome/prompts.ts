...
- 可以使用人生终章、完整人生回顾和人生志铭等表达。`;
  ...
  const futureTrendRule = isReflection
    ...
    : "此字段在死亡终局中表示遗留影响：trend 和 reason 都必须使用已经发生的回顾语气，只能回顾历史已经证明的作品、关系或制度影响；不得使用‘你将/你会/你仍/你继续/你开始’，不得推演死后的法律程序、债务承担者、采用范围或影响人数。";
...
- mortality 报告的每一个字符串字段都必须是回顾语气；逐字段自检并删除“你将、你会、你仍、你继续、你开始、请继续、你应该、你需要、下一阶段”。
- 若历史只写“周边学校、邻县老师、共建团队”等局部对象，报告必须沿用这些对象，不得扩写成“多所学校、多个县域、全国、无数、一代代、广泛采用”。
